...
function ChatBot({ filePath }) {
	...
	const handleSubmit = () => {
		if (prompt.trim() !== "") {
			setChats([
				...chats,
				<Chat key={chats.length} isUser={true} userPrompt={prompt} />,
			]);
			chat();
			setPrompt("");
		}
	...
	return (
		<>
			<div className="min-h-screen bg-slate-900 w-[50vw] flex flex-col justify-center items-center text-white overflow-auto ">
				<div
					id="customScrollBar"
					className=" flex flex-col justify-between h-[80vh] w-[100%] text-sm sm:text-md mb-5 rounded-md px-2 overflow-auto overflow-y-scroll"
				>
					<div>
						{chats}
						{loading && <Loader />}
				...
				<div
					className={`flex justify-center items-center bg-slate-800  shadow-[0_0px_10px_-1px_rgba(0,0,0,0.2)] px-4 py-3 mb-5 rounded-md w-[80%] lg:w-[85%] ${filePath === null ? "cursor-not-allowed" : ""}`}
				>
					<input
						type="text"
						value={prompt}
